...
class PlayState extends MusicBeatState
{
	...
	function startSong():Void
	{
		...
		if(autoUpdateRPC) DiscordClient.changePresence(detailsText, discordStateText, iconP2.getCharacter(), true, songLength);
		#end
	...
	{
		#if desktop
		if (health > 0 && !paused && autoUpdateRPC) DiscordClient.changePresence(detailsPausedText, discordStateText, iconP2.getCharacter());
		#end
	...
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		if(!ClientPrefs.data.healthBarTweening) {healthBar.percent = (newPercent != null ? newPercent : 0);}

		var healthPercent = (newPercent != null ? newPercent : health*50);

		iconP1.animation.curAnim.curFrame = (healthPercent < 20) ? 1 : 0; //If health is under 20%, change player icon to frame 1 (losing icon), otherwise, frame 0 (normal)
		iconP2.animation.curAnim.curFrame = (healthPercent > 80) ? 1 : 0; //If health is over 80%, change opponent icon to frame 1 (losing icon), otherwise, frame 0 (normal)
		return health;
	}
	...
	function openPauseMenu()
	{
		...
		#if desktop
		if(autoUpdateRPC) DiscordClient.changePresence(detailsPausedText, discordStateText, iconP2.getCharacter());
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				if(autoUpdateRPC) DiscordClient.changePresence("Game Over - " + detailsText, discordStateText, iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
	...
	private function keysCheck():Void
	{
		...
		for (key in keysArray)
		{
			...
			if(controls.controllerMode || inputSystem != null)
			{
